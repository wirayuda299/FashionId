...
const LadiesProduct: FC = () => {
  ...
  const { state: { newArrivals, selectedLadiesCategory } } = useStateContext();

  const { data, isError, isLoading } = useFetchProducts('female', selectedSort, selectedLadiesCategory, newArrivals);
  if (isError) {
    return (
      ...
      <div className="container mx-auto">
        <div className='w-full '>
          <div className='w-full text-center'>
            <h2 className='font-bold text-xl text-black sm:text-2xl'>{selectedLadiesCategory === 'All' ? title : `All ${selectedLadiesCategory}`}</h2>
            <div className='flex h-16  max-w-5xl justify-between px-9 sm:px-0  mx-auto items-center '>
              ...
              <div className='flex items-center space-x-3'>
                ...
                <div className='flex space-x-3'>
                  <button
                    className={`text-xs sm-text-sm  ${isModel ? 'border-b border-black' : ''}`}
