...
const Detail: FC = () => {
	...
	const { data, isError, isLoading } = useQuery(
		...
		async () => {
			...
			setRecommendedProduct(
				response.filter((item) => item._id !== id && item.gender === gender)
			);
	...
	const addToCart = (product: Products) => {
		setClicked(true);
		const checkDuplicates = cartItems.find(
					...
					<div className='container mx-auto h-full'>
						<div className='grid grid-cols-1 lg:grid-cols-2'>
							...
							<div className='w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0'>
								<small className='text-sm title-font text-gray-500 tracking-widest pb-1'>
									H&M
								</small>
								<h1 className='text-gray-900 text-3xl title-font font-bold mb-1 capitalize pb-2'>
									{product?.title}
								</h1>
								...
								<div className='flex items-center justify-between'>
									<span className='title-font font-medium md:font-bold text-2xl text-gray-900'>
										${product?.price}
									</span>
									<button
